...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let allKeysData = []; // Conterrà sia lettere che fonemi
    ...
    async function initializeManager() {
        ...
        try {
            const [lettersRes, phonemesRes] = await Promise.all([
                fetch(`${dataPath}letters.json`),
                fetch(`${dataPath}phonemes-new.json`)
            ]);
            const lettersJson = await lettersRes.json();
            const phonemesJson = await phonemesRes.json();
            const letters = lettersJson.layout.flatMap(row => row.keys);
            const phonemes = phonemesJson.layout.flatMap(row => row.keys);
            allKeysData = [...letters, ...phonemes]; // Unisce i dati in un unico array
        } catch (err) {
            console.error("Errore caricamento dati per Game 2", err);
        }
    ...
    async function togglePhotoMode(e) {
        ...
        if (isPhotoModeOn) {
            ...
            // Qui potresti voler disattivare altre modalità, ad esempio:
            // if (isWordyModeOn) {
            //     await toggleWordyMode({ stopPropagation: () => {} });
            // }
            // if (isSimboModeOn) { // Se isSimboModeOn e toggleSimboMode esistono
            //     await toggleSimboMode({ stopPropagation: () => {} });
            // }
        } else {
    ...
    async function handleKeyPress(event) {
        ...
        stopLetterAttentionSequence(); // Ferma l'animazione di attenzione
        const { char } = event.detail; // Prende il carattere dall'evento
        const letterData = allKeysData.find(l => l.ch === char && l.word); // Cerca i dati della lettera (assicurandosi che abbia una parola associata)
    ...
    async function animateLetters(letterData, placeholders) {
        for (let i = 0; i < letterData.word.length; i++) {
            ...
            await flyToTarget(sourceKey, targetPlaceholder, 'letter-style', typyMaestro, char);
    ...
    async function animatePhonemes(letterData, placeholders, letterPlaceholders) {
        ...
        for (let i = 0; i < phonemes.length; i++) {
            ...
            if (sourceKey && targetPlaceholder) {
                await flyToTarget(sourceKey, targetPlaceholder, 'phoneme-style', lookyMaestro, phoneme);
            }
        }
    }

    /**
     * Normalizza un percorso audio per gestire le esecuzioni da sottocartelle (es. /games/).
     * @param {string} path - Il percorso audio originale.
     * @returns {string|null} Il percorso audio completo e corretto.
     */
    function buildAudioPath(path) {
        if (!path) return null;
        const prefix = window.location.pathname.includes('/games/') ? '../Audio/Tastiere/' : 'Audio/Tastiere/';
        // Rimuove un eventuale prefisso per evitare duplicazioni e poi lo aggiunge
        const cleanPath = path.replace(/^(..\/)?/, '');
        return `${prefix}${cleanPath}`;
    }


    async function flyToTarget(source, target, styleClass, intermediateTarget, char) {
        console.log("Funzione flyToTarget chiamata.");
        console.log("Dati ricevuti:", { source, target, char });
        console.log("Entrato nella funzione flyToTarget con i seguenti parametri:", { source, target, styleClass, intermediateTarget, char });
        if (!keyData) {
            console.error("Nessun dato trovato per il carattere:", char);
        } else {
            console.log("Dati trovati per il carattere:", keyData);
        }

        if (!source || !target) {
            console.error("Elemento sorgente o target mancante. Impossibile continuare l'animazione.");
            return;
        }

        console.log("Elemento sorgente e target validi. Procedo con l'animazione.");
        ...
        const prom = new Promise(resolve => {
            ...
            // Trova i dati completi della lettera/fonema (incluso il suono)
            const keyData = allKeysData.find(k => k.ch === char);
            ...
            // Riproduce il suono usando il percorso dai dati JSON caricati
            // Se keyData.sound non esiste (come per i fonemi), lo costruisce dinamicamente.
            let soundPath = keyData?.sound;
            if (!soundPath && keyData?.ch) {
                soundPath = `Audio/Tastiere/${keyData.ch}.wav`;
            }

            console.log("Verifica: il flusso di esecuzione è entrato nella funzione flyToTarget.");
            console.log("Dati della lettera:", keyData);
            if (!keyData || !keyData.sound) {
                console.warn("Dati della lettera mancanti o percorso audio non trovato.");
            } else {
                console.log("Percorso audio trovato:", keyData.sound);
            }

            console.log("Verifica: buildAudioPath restituisce:", buildAudioPath(soundPath));
            console.log("Verifica: il metodo play verrà chiamato per il file:", buildAudioPath(soundPath));

            console.log("Tentativo di riprodurre il suono per il carattere:", char);
            if (soundPath) {
                const correctedPath = buildAudioPath(soundPath);
                console.log(`Percorso audio costruito: ${correctedPath}`);
                const audio = new Audio(correctedPath);
                audio.addEventListener('canplaythrough', () => {
                    console.log(`File audio pronto per la riproduzione: ${correctedPath}`);
                });
                audio.addEventListener('error', () => {
                    console.error(`Errore durante il caricamento del file audio: ${correctedPath}`);
                });
                audio.play().catch(err => console.error("Errore durante la riproduzione del file audio:", err));
            } else {
                console.warn("Percorso audio non trovato per il carattere:", char);
            }
